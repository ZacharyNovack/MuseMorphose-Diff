...
from model.musemorphose import MuseMorphose, MuseDiff

from utils import pickle_load, numpy_to_tensor, tensor_to_numpy
from remi2midi_ms import remi2midi
...
def generate_on_latent_ctrl_vanilla_truncate(
        model, latents, diff_cls, event2idx, idx2event, 
  ...
  diff_placeholder = torch.zeros(max_events, 1, dtype=int).to(device)
  ...
  print ('[info] diff cls: {}'.format(diff_cls))

  if primer is None:
    generated = [event2idx['bar']]
  else:
    generated = [event2idx[e] for e in primer]
    latent_placeholder[:len(generated), 0, :] = latents[0].squeeze(0)
    diff_placeholder[:len(generated), 0] = diff_cls[0]
  ...
  while generated_bars < target_bars:
    ...
    diff_placeholder[len(generated)-1, 0] = diff_cls[ generated_bars ]
    p#olyph_placeholder[len(generated)-1, 0] = polyph_cls[ generated_bars ]

    dec_seg_emb = latent_placeholder[:len(generated), :]
    dec_diff_cls = diff_placeholder[:len(generated), :]
    #dec_polyph_cls = polyph_placeholder[:len(generated), :]

    # sampling
    with torch.no_grad():
      logits = model.generate(dec_input, dec_seg_emb, dec_diff_cls)
    ...
    if len(generated) > max_events or (word_event == 'eos' and generated_bars == target_bars - 1):
      generated_bars += 1
      generated.append(event2idx['bar'])
    ...
    if cur_input_len == max_input_len:
      ...
      diff_placeholder[:len(generated)-1, 0] = diff_placeholder[cur_input_len-truncate_len:cur_input_len-1, 0]
...
if __name__ == "__main__":
  ...
  if 'pad' not in dset.event2idx:
    dset.event2idx['pad'] = dset.vocab_size
    dset.idx2event[dset.vocab_size] = 'pad'
  mconf = config['model']
  model = MuseDiff(
    mconf['enc_n_layer'], mconf['enc_n_head'], mconf['enc_d_model'], mconf['enc_d_ff'],
    mconf['dec_n_layer'], mconf['dec_n_head'], mconf['dec_d_model'], mconf['dec_d_ff'],
    mconf['d_latent'], mconf['d_embed'], dset.vocab_size,
    d_diff_emb=mconf['d_diff_emb'],
    cond_mode=mconf['cond_mode']
  ).to(device)
  model.eval()
  #ith open(ckpt_path, 'rb') as f:
    #model.load_state_dict(torch.load(ckpt_path, map_location='cpu'))
  model.load_state_dict(pickle_load(ckpt_path))
  ...
  for p in pieces:
    ...
    orig_d_cls_str = ''.join(str(c) for c in p_data['diff_cls_bar'])
    ...
    np.save(orig_out_file + '-DIFFCLS.npy', p_data['diff_cls_bar'])
    ...
    d_cls_diff = eval(control)(n_samples_per_piece)
    ...
    #   p_rfreq_cls = (p_data['rhymfreq_cls_bar'] + r_cls_diff[samp]).clamp(0, 7).long()
      p_diff_cls = (p_data['diff_cls_bar'] + d_cls_diff[samp]).clamp(0, 2).long()
    ...
    #   ))   
      out_file = os.path.join(out_dir, 'id{}_bar{}_sample{:02d}_diff{}'.format(
        p, p_bar_id, samp + 1,
        '+{}'.format(d_cls_diff[samp]) if d_cls_diff[samp] >= 0 else d_cls_diff[samp]
      ))         
      ...
      song, t_sec, entropies = generate_on_latent_ctrl_vanilla_truncate(
                                  model, p_latents, p_diff_cls, dset.event2idx, dset.idx2event,
    ...
    #   np.save(out_file + '-RHYMCLS.npy', tensor_to_numpy(p_rfreq_cls))
      np.save(out_file + '-DIFFCLS.npy', tensor_to_numpy(p_diff_cls))
