...
def train_model(epoch, model, dloader, dloader_val, optim, sched):
  ...
  for batch_idx, batch_samples in enumerate(dloader):
    ...
    batch_diff_cls = batch_samples['diff_cls'].permute(1, 0).to(device)
    ...
    mu, logvar, dec_logits = model(
      ...
      batch_inp_bar_pos, batch_diff_cls,
    ...
    if not trained_steps % ckpt_interval:
      with open(os.path.join(params_dir, f'{runN}-params.pk'), 'wb') as f:
        pk.dump(model.state_dict(), f)
      with open(os.path.join(optim_dir, f'{runN}-opt.pk'), 'wb') as f:
        pk.dump(optim.state_dict(), f)
...
def validate(model, dloader, n_rounds=3, use_attr_cls=True):
  model.eval()
  loss_rec = []
  kl_loss_rec = []
  ...
  with torch.no_grad():
    for i in range(n_rounds):
      ...
      for batch_idx, batch_samples in enumerate(dloader):
        ...
        mu, logvar, dec_logits = model(
          batch_enc_inp, batch_dec_inp, 
          batch_inp_bar_pos, batch_diff_cls, #batch_polyph_cls,
          padding_mask=batch_padding_mask
        )
...
if __name__ == "__main__":
  ...
  dset_val = MSREMIFullSongTransformerDataset(
    config['data']['val_dir'], config['data']['vocab_path'], 
    do_augment=False, 
    model_enc_seqlen=config['data']['enc_seqlen'], 
    model_dec_seqlen=config['data']['dec_seqlen'], 
    model_max_bars=config['data']['max_bars'],
  ...
  model = MuseDiff(
    ...
    d_diff_emb=mconf['d_diff_emb'], #d_rfreq_emb=mconf['d_rfreq_emb'],
